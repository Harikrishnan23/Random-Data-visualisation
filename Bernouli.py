...
while(i < maxNumber):
    ...
    i = i+1
    f2 = open('randBer.txt', 'w')
len = 0
for x in randomNumbers:
    len+=len+1
randomNumbers.sort()
propabilityValue = float(input("Enter the probability value()"))
def bernoliDistribution():
    y=[]
    f = open('bernouli.txt', 'w')
    i=0
    while(i < maxNumber):
        y.append(round((1 - propabilityValue),4))
        f.write(str(y[i]) + "\n")
